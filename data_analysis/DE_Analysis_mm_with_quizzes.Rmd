...
myQuestions3 = [
  ...
  question: "'Extra credit' question: Rerun the differential expression analysis using a expression filtering cutoff of 1 instead of 2.  How many significantly DE genes do you get for the comparison of WT C and WT NC?",
    answers: {
      a: "`r y.1 <- voom(d0[-which(apply(cpm(d0), 1, max) < 1),], mm); fit.1 <- lmFit(y.1, mm); tmp.1 <- contrasts.fit(fit.1, contr); tmp.1 <- eBayes(tmp.1); format(length(which(topTable(tmp.1, n = Inf, sort.by = "P")$adj.P.Val < 0.05)), big.mark = ",")`",
      b: "`r format(length(which(top.table$adj.P.Val < 0.05)), big.mark = ",")`",
      c: "0"
    },
    correctAnswer: "a"
  }
...
myQuestions4 = [
  ...
  },
  {
    ...
    answers: {
      a: "`r mm.1 <- model.matrix(~0 + group + mouse); y.1 <- voom(d, mm.1); fit.1 <- lmFit(y.1, mm.1); tmp.1 <- contrasts.fit(fit.1, contrasts = makeContrasts(groupWT.NC - groupKOMIR150.NC, levels = colnames(coef(fit.1)))); tmp.1 <- eBayes(tmp.1); format(length(which(topTable(tmp.1, n = Inf, sort.by = "P")$adj.P.Val < 0.05)), big.mark = ",")`",
      b: "`r mm.1 <- model.matrix(~0 + group + mouse); y.1 <- voom(d, mm.1); fit.1 <- lmFit(y.1, mm.1); tmp.1 <- contrasts.fit(fit.1, contrasts = makeContrasts(groupWT.C - groupKOMIR150.C, levels = colnames(coef(fit.1)))); tmp.1 <- eBayes(tmp.1); format(length(which(topTable(tmp.1, n = Inf, sort.by = "P")$adj.P.Val < 0.05)), big.mark = ",")`",
      c: "`r mm.1 <- model.matrix(~0 + group + mouse); y.1 <- voom(d, mm.1); fit.1 <- lmFit(y.1, mm.1); tmp.1 <- contrasts.fit(fit.1, contrasts = makeContrasts(groupWT.C - groupWT.NC, levels = colnames(coef(fit.1)))); tmp.1 <- eBayes(tmp.1); format(length(which(topTable(tmp.1, n = Inf, sort.by = "P")$adj.P.Val < 0.05)), big.mark = ",")`",
    },
    correctAnswer: "a"
  },
  {
